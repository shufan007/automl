...
class my_Layernorm(nn.Module):
    ...
    def forward(self, x):
        x_hat = self.layernorm(x).type(x.dtype)
...
class series_decomp_multi(nn.Module):
    ...
    def forward(self, x):
        ...
        moving_mean = torch.sum(moving_mean * nn.Softmax(-1)(self.layer(x.unsqueeze(-1))),dim=-1).type(x.dtype)
        res = x - moving_mean
        return res, moving_mean
...
class Decoder(nn.Module):
    ...
    def forward(self, x, cross, trend: Optional[torch.Tensor]=None,
                x_mask: Optional[torch.Tensor]=None,
                cross_mask: Optional[torch.Tensor]=None):
