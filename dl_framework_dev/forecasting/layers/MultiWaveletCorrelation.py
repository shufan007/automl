...
class MultiWaveletTransform(nn.Module):
    ...
    def __init__(self, ich=1, k=8, alpha=16, c=128,
                 nCZ=1, L=0, base='legendre', attention_dropout=0.1, complex_split=False):
        ...
        self.ich = ich
        self.MWT_CZ = nn.ModuleList(MWT_CZ1d(k, alpha, L, c, base, complex_split=complex_split) for i in range(nCZ))

    def forward(self, queries, keys, values, attn_mask: Optional[torch.Tensor]=None):
...
class FourierCrossAttentionW(nn.Module):
    ...
    def forward(self, q, k, v, mask:Optional[torch.Tensor]=None):
        ...
        out = torch.fft.irfft(out_ft / self.in_channels / self.out_channels, n=xq.size(-1)).permute(0, 3, 2, 1).type(original_type)
        # size = [B, L, H, E]
        return (out, None)
...
class sparseKernelFT1d(nn.Module):
    def __init__(self,
        ...
        if self.complex_split:
            self.weightsa = nn.Parameter(self.scale * torch.rand(c * k, c * k, self.modes1), requires_grad=True)
            self.weightsb = nn.Parameter(torch.rand(c * k, c * k, self.modes1), requires_grad=True)
        else:
    ...
    def forward(self, x):
        ...
        out_ft[:, :, :l] = self.compl_mul1d(x_fft[:, :, :l], weights1[:, :, :l])
        x = torch.fft.irfft(out_ft, n=N).type(original_type)
...
class MWT_CZ1d(nn.Module):
    def __init__(self,
        ...
        self.A = sparseKernelFT1d(k, alpha, c, complex_split=complex_split)
        self.B = sparseKernelFT1d(k, alpha, c, complex_split=complex_split)
        self.C = sparseKernelFT1d(k, alpha, c, complex_split=complex_split)
